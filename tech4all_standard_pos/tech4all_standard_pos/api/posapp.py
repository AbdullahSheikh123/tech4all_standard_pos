...
from tech4all_standard_pos.tech4all_standard_pos.doctype.pos_coupon.pos_coupon import check_coupon_code
...
def get_opening_dialog_data():
    ...
    for pos_profile in data["pos_profiles_data"]:
        ...
        if branch_account and cost_center:
            ...
            current_pakistan_time = datetime.now(pakistan_tz)
            current_date = current_pakistan_time.date()
...
def get_items(
    ...
    def _get_items(
        ...
        custom_hide_straight_items = pos_profile.get("custom_hide_straight_items")
        ...
        if not posa_show_template_items and not posa_hide_variants_items and not custom_hide_straight_items:
                    # Show only simple items (items that are neither templates nor variants)
            condition += " AND (has_variants = 0 AND variant_of IS NULL)"

        elif posa_show_template_items and not posa_hide_variants_items and not custom_hide_straight_items:
            # Show template, variants, and simple items
            condition += " AND (has_variants = 1 OR variant_of IS NOT NULL OR (has_variants = 0 AND variant_of IS NULL))"

        elif posa_show_template_items and posa_hide_variants_items and not custom_hide_straight_items:
            # Show only template and simple items (hide variants)
            condition += " AND (has_variants = 1 OR (has_variants = 0 AND variant_of IS NULL))"

        elif posa_show_template_items and posa_hide_variants_items and custom_hide_straight_items:
            # Show only template items
            condition += " AND has_variants = 1"
        ...
        if items_data:
            ...
            for item in items_data:
                ...
                # Get first variant's price if item is a template
                if item.has_variants:
                    first_variant = frappe.db.get_value(
                        "Item",
                        {"variant_of": item_code, "disabled": 0},
                        ["name"],
...
@frappe.whitelist()
def get_Table_names(pos_profile, order_type=None):
    _pos_profile = json.loads(pos_profile)
    ttl = _pos_profile.get("posa_server_cache_duration")
    if ttl:
        ttl = int(ttl) * 60
    ...
    @redis_cache(ttl=ttl or 1800)
    def __get_Table_names(pos_profile, order_type):
        return _get_Table_names(pos_profile, order_type)

    def _get_Table_names(pos_profile, order_type):
        pos_profile = json.loads(pos_profile)
        
        # Fetch branch linked to the POS profile
        branch = frappe.get_value("Branch", {"pos_profile": pos_profile.get("name")}, "name")
        
        if not branch:
            return []  # Return empty list if no branch is found

        # Prepare filters
        filters = {"status": "Available", "branch": branch}
        
        # Add order_type filter if provided
        if order_type:
            filters["order_type"] = order_type
        
        tables = frappe.get_all(
            "Table Management",
            fields=["name", "status", "floor_no", "table_no", "no_of_seats"],
            filters=filters,
            order_by="name"
        )
        return tables
    ...
    if _pos_profile.get("posa_use_server_cache"):
        return __get_Table_names(pos_profile, order_type)
    else:
        return _get_Table_names(pos_profile, order_type)
...
def update_invoice(data):
    ...
    if not data.get("posa_pos_opening_shift"):
        ...
        if open_vouchers:
            data["posa_pos_opening_shift"] = open_vouchers[0]["name"]
        ...
        for bundle_data in bundles_data:
            ...
            if len(items_list) == 1 and not item_specifics:
                ...
                created_bundles.append({
                    ...
                    "bundle_doc": frappe.get_doc("Item", variant_item["item_code"]).as_dict()
                })
                    ...
                    if all_items_matched:
                        ...
                        if len(bundle_specifics) == len(item_specifics):
                            ...
                            for i in range(len(bundle_specifics)):
                                if bundle_specifics[i].item_specifics_name != item_specifics[i]["name"]:
                                    all_specifics_matched = False
                                    break
                            
                            if all_specifics_matched:
                                existing_bundle_found = True
                                matched_bundle_doc = bundle_doc
                                break
            ...
            if existing_bundle_found:
                ...
                created_bundles.append({
                    "item_code": matched_bundle_doc.name,
                    "qty": items_list[0]["qty"],
                    "rate": total_rate,
                    "bundle_doc": matched_bundle_doc.as_dict()
                })
            else:
                # Create a new service item for the bundle
                first_item_code = items_list[0]["item_code"]
                firs_item_name = items_list[0]["item_name"]
                ...
                service_item_doc.item_name = firs_item_name
                ...
                created_bundles.append({
                    ...
                    "qty": items_list[0]["qty"],
                    "rate": total_rate,
                    "bundle_doc": frappe.get_doc("Product Bundle", new_bundle.name).as_dict()
                })
...
@frappe.whitelist()
def search_orders(company, currency, pos_profile=None, order_name=None):
    ...
    orders_list = frappe.get_list(
        "Sales Order",
        filters=filters,
        fields=["name", "delivery_date", "status", "branch"],
        limit_page_length=0,
        order_by="customer",
    )
